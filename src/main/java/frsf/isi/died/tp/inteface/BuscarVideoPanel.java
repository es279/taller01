...
public class BuscarVideoPanel extends JPanel {
	private ArrayList<Video> resultadoBusqueda;
	private Suscriptor usuario;
	
	public BuscarVideoPanel(Suscriptor sus, MainFrame mf) {
		this.usuario = sus;
		resultadoBusqueda = new ArrayList<Video>();
			...
			public void actionPerformed(ActionEvent e) {
				if(tablaDeResultados.getSelectedRow() != -1) {
					...
					if (!existe) {
						...
						JOptionPane.showMessageDialog(null, "El Video seleccionado ya tiene un árbol asignado", "Info",JOptionPane.NO_OPTION);
					}
				}
			}
		});
		...
		actualizar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(rowIndex != -1) {
					mf.cambiar( new ActualizarVideoPanel( resultadoBusqueda.get(rowIndex), mf) );
				}
		...
		eliminar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				int rowIndex = tablaDeResultados.getSelectedRow();
				if(rowIndex != -1) {
					boolean exito = RegisterManager.eliminarVideo( (String) modelo.getValueAt(rowIndex, 0) ) ;
					if(exito) {
		...
		buscar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(Utilidades.validarCriterios(titulo.getText(),calificacion.getText(),fechaDesde,fechaHasta)) {
					resultadoBusqueda = RegisterManager.cargarVideosConCriterios(titulo.getText(), calificacion.getText(), tema.getSelectedItem().toString(), fechaDesde, fechaHasta);
					ordenarArrayResultado((String) tipoOrdenResultado.getSelectedItem());
					actualizarTabla(modelo);
				}
			}
