...
public class BibliotecaABB implements Biblioteca {
	...
	private void ordenarPorPrecio() {
		...
		List<MaterialCapacitacion> casa = this.materiales.inOrden();
		casa.sort(this.comparaPrecio);
		
		ArbolBinarioBusqueda arb = new ArbolBinarioBusqueda(casa.get((int)(casa.size()/2)),this.comparaPrecio);
		casa.remove((int)(casa.size()/2));
		for(MaterialCapacitacion mat : casa) {
			arb.add(mat);
		}
		this.materiales=arb;
	...
	private void ordenarPorTitulo() {
		...
		List<MaterialCapacitacion> casa = this.materiales.inOrden();
		casa.sort(this.comparaTitulo);
		
		ArbolBinarioBusqueda arb = new ArbolBinarioBusqueda(casa.get((int)(casa.size()/2)),this.comparaTitulo);
		casa.remove((int)(casa.size()/2));
		for(MaterialCapacitacion mat : casa) {
			arb.add(mat);
		}
